...
class Threading_func():
  def __init__(self):
        ...
        print("Play  wave file %s." % wavfile)
        # Open wave file (should be mono channel)
        self.wf = wave.open(r'C:\Users\jonat\Downloads\auth_mono_large.wav','rb')
...
################################################################
  def Pythread(self):
        ...
        for i in range(0, num_blocks):
                ...
                self.y1 = scipy.signal.lfilter(self.b_l,self.a_l,x)        #LP
                self.y2 = scipy.signal.lfilter(self.b_h,self.a_h,self.y1)        #HP
                self.y3 = scipy.signal.lfilter(self.b_p,self.a_p,self.y2)        #PF
